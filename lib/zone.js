...
Zone.prototype.rgb = function(expr, callback) {
	...
	if (typeof (expr) === "string") {
		...
		if (result) {
			console.log(result);
			return this.rgb(parseInt(result[1], 16), callback);
		}

		reg = /^rgb\(([0-9]{1,3}%?),([0-9]{1,3}%?),([0-9]{1,3}%?)\)$/i;
		result = reg.exec(expr);
	...
	return callback("Invalid color code ('" + expr + "'");
};
...
function brightness(milight, v, callback) {
	...
	console.log("Set brightness " + brightnessKey);
	...
	milight._sync(function(callback) {
		async.eachSeries(zones, function(zone, callback) {
			...
			milight._selectZone(zone, function(error) {
				...
				// console.log("Set Hue to ", h, " on zone ", zone);
...
Zone.prototype.on = function(callback) {
	...
	milight._sync(function(callback) {
		async.eachSeries(zones, function(zone, callback) {
			milight._on(zone, true, callback);
		}, callback);
	}, callback);
};
